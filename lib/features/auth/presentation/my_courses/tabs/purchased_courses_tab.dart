...
// Verified imports via reasoning
...
import '../../courses/courses_screen.dart'; // ✅ Import for Explore More
import '../course_player_screen.dart';
import '../my_courses_screen.dart'; // Import for constants

class PurchasedCoursesTab extends StatefulWidget {
  const PurchasedCoursesTab({super.key});

  @override
  State<PurchasedCoursesTab> createState() => _PurchasedCoursesTabState();
}

class _PurchasedCoursesTabState extends State<PurchasedCoursesTab> with AutomaticKeepAliveClientMixin {
  ...
  bool _showRecommendedCourses = false;
  ...
  Future<void> _fetchCourses() async {
    if (_courses.isNotEmpty && !_showRecommendedCourses) return;
    ...
    try {
      final purchasedCourses = await _coursesRepository.getPurchasedCourses();
      ...
      if (purchasedCourses.isEmpty) {
        ...
        if (mounted) {
          setState(() {
            _courses = purchasedCourses;
            _showRecommendedCourses = false;
            _isLoading = false;
          });
        }
      }
    ...
    return RefreshIndicator(
      ...
      child: _showRecommendedCourses
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, i) => _buildCourseCard(_courses[i], isRecommended: false),
            ),
    );
  }
}
